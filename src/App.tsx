         ...
         <Col md={12}>
          <h3>Grocery List</h3>
           <form onSubmit={handleSubmit}>
              ...
              />
              <button type="submit" className="btn btn-primary">{`${isEditing ? "Edit" : "Create"}`}</button>
           </form>
         </Col>
         <Col md={12}>
          {list.map((item: any) => {
              const {id, title} = item
              return <div key={id} style={{display: "flex", justifyContent: "center", fontSize: "25px"}}>
                  <p>{title}</p>
